...
public class HomePageController implements Initializable {
   ...
   void setFriend()
   {
        EntityManagerFactory emf =
            Persistence.createEntityManagerFactory("dataFriend.odb");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        String checkdata;
        TypedQuery<dataFriend> query =
        em.createQuery("SELECT p FROM dataFriend p", dataFriend.class);
        for(dataFriend a:query.getResultList()){
            checkdata=a.getX();
            System.out.println(checkdata);
            friendList_name.add(checkdata);
        }
   ...
   {  
       ...
       setFriend();
       friend_list.setItems(friendList_name); //print show
       //Generate Calendar
   ...
   void generateCalendar(int month,int year){
    ...
    private void signOut(ActionEvent event) {
        ...
        void addFriendData(String x){
        EntityManagerFactory emf =
            Persistence.createEntityManagerFactory("dataFriend.odb");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        dataFriend p = new dataFriend(x);
        em.persist(p);
        em.getTransaction().commit();
        long id=0;
        TypedQuery<dataFriend> query =
        ...
        friendList_name.add(x);
        System.out.println("Add : "+x);
        em.close();
    }
    
    void delFriendData(String data){
        EntityManagerFactory emf =
            Persistence.createEntityManagerFactory("dataFriend.odb");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        ...
        for(dataFriend a:query.getResultList()){
            ...
            {  
                ...
                int deletedCount = em.createQuery("DELETE FROM dataFriend where id="+Integer.toString((int) id)).executeUpdate();
            }
        }
        em.getTransaction().commit();
        em.close();
    ...
    private void deleteFriend(ActionEvent event) {
        ...
        for(String name : friendList_name){
            ...
            {
                delFriendData(select_Friendname);
